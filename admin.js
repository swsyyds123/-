...
// 获取登录密码
const getAuth = () => JSON.parse(localStorage.getItem('currentUser') || '{}').password || '';

// --- 核心：云端同步 (强制推送) ---
async function syncToKV() {
    showToast("正在同步云端...", 10000); // 持续显示加载
    const payload = {
        password: getAuth(),
    ...
    try {
        ...
        if (result.success) {
            showToast("✅ 同步成功！云端已更新");
        } else {
            alert("❌ 同步失败：" + result.error);
        }
    } catch (e) {
        alert("❌ 网络错误：请检查 API 地址是否正常");
    }
}
    ...
    container.innerHTML = window.state.categories.map(c => `
        <div class="list-item" draggable="true" ondragstart="handleDragStart(event)" ondragover="handleDragOver(event)" ondrop="handleDrop(event)" data-id="${c.id}">
            <div class="list-item-info">
                <i class="fas fa-grip-vertical" style="color:#888; margin-right:15px; cursor:move"></i>
                <span>${c.name}</span>
            </div>
...
function renderSettingsPanel() {
    ...
    if (window.state.settings.backgroundImage) {
        document.getElementById('bgPreviewContainer').innerHTML = `<img src="${window.state.settings.backgroundImage}" style="max-height:120px; border-radius:8px">`;
    }
}

// --- 全局交互函数 (解决 HTML onclick 失效) ---
window.closeModals = () => {
...
window.handleDrop = async (e) => {
    ...
    if (target !== draggedElement) {
        ...
        if (draggedIdx < targetIdx) target.after(draggedElement); else target.before(draggedElement);
        const newItems = [...list.querySelectorAll('.list-item')];
        newItems.forEach((item, index) => {
            window.state.categories.find(c => c.id === item.dataset.id).order = index;
        });
        await syncToKV();
    }
};
...
window.onload = () => {
    ...
    document.getElementById('previewBgBtn').onclick = () => {
        ...
        if (url) {
            document.getElementById('bgPreviewContainer').innerHTML = `<img src="${url}" style="max-height:120px; border-radius:8px">`;
        }
    };
...
function showToast(m, time = 2500) {
...
function getDefaultLinks() {
    return [
        ...
        {"id":"103","categoryId":"9","name":"最伟大的AI","url":"https://flowith.io/blank","order":11},
